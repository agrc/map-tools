define(['dojo/text!./templates/FindAddress.html', 'dojo/_base/declare', 'dojo/_base/Color', 'dojo/_base/lang', 'dojo/_base/array', 'dojo/dom-class', 'dojo/query', 'dojo/topic', 'dojo/on', 'dojo/string', 'dijit/_WidgetBase', 'dijit/_TemplatedMixin', 'esri/symbols/SimpleMarkerSymbol', 'esri/geometry/Point', 'esri/geometry/scaleUtils', 'esri/graphic', 'esri/request', 'esri/SpatialReference'], function (template, declare, Color, lang, array, domClass, query, topic, on, dojoString, _WidgetBase, _TemplatedMixin, SimpleMarkerSymbol, Point, scaleUtils, Graphic, esriRequest, SpatialReference) {
    // description:
    ...
    return declare([_WidgetBase, _TemplatedMixin], {
        ...
        geocodeAddress: function geocodeAddress() {
            ...
            this.request = this._invokeWebService({
                street: address,
                zone: zone
            }).then(lang.hitch(this, '_onFind'), lang.hitch(this, '_onError'));

            return false;
        },

        _invokeWebService: function _invokeWebService(geocode) {
            // summary:
        ...
        _isValid: function _isValid(textBox) {
            // summary:
        ...
        _geocoding: function _geocoding() {},

        _done: function _done() {},

        onFind: function onFind() {},

        _onFind: function _onFind(response) {
            // summary:
            ...
            if (response.status === 200) {
                ...
                if (this.map) {
                    var point = new Point(response.result.location.x, response.result.location.y, new SpatialReference({
                        wkid: this.wkid
                    }));
                    ...
                    if (this.map.getLevel() > -1) {
                        ...
                        this.map.centerAndZoom(point, scaleUtils.getScale(this.map.extent, this.map.width, this.wkid) / this.zoomLevel);
                    }
...
//# sourceMappingURL=FindAddress.js.map
